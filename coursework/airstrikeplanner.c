...
Planner *loadFile(Planner *p, Planner *head)
{
    ...
    //get the pointer back to the beginning of the file
    rewind(fp);
    ...
    while (1)
    {
        ...
        if (flag)
        {
            ...
            //if the heap is run ou of the memory, the profgram will exit with argument -1
            if (pt == NULL)
            {
                fclose(fp);
                exit(-1);
...
Planner *destroy(Planner *head, Planner *oldp)
{
    ...
    //count the number of target which is within the range of attack
    while (p->next != NULL)
    {
        if (getDistance(p->latitude, pla, p->longitude, plo) < ratio)
        {
    ...
    //reset the pointer
    p = head;
    //if no target can be matched, show warning，otherwise show the number if target in the area
    if (targetNumber > 0)
    {
    ...
    //free the memory and adjust the last node of the link list
    while (p->next != NULL)
    {
        if (getDistance(p->latitude, pla, p->longitude, plo) < ratio)
        {
    ...
    //return the current last node to the main function
    return p;
}
...
void inputFileName(char fileName[40])
{
    ...
    str[i] = '\0';
    //Because fopen() function will ignore the ' ' behind the string, so it should be handled. Thus, if the user input a string like "target1.txt   " is should be recognized as invalid input.
    if (str[i - 1] == ' ')
    {
        return;
    }
...
int menuInput()
{
    ...
    while (1)
    {
        //if the first character is '\n', the program will wait for the next input
        if (c1 == '\n')
        {
            c1 = getchar();
            continue;
        }
        //if the first character is not within the range of 1~9, return the fail message(-1)
        if (c1 < '1' || c1 > '6')
        {
            while ((ch = getchar()) != '\n')
                ;
    ...
    //receive the second user input character
    c2 = getchar();
